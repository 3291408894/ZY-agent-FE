...
import { sendResourceToClass } from '@/api/modules/teachingResource'
...
async function handleSend() {
  ...
  sending.value = true
  try {
    const result = await sendResourceToClass(selectedResourceId.value!, {
...
function openForResource(resourceId: string) {
  selectedResourceId.value = resourceId
  open()
}

defineExpose({ openForResource })
</script>
...
<template>
  ...
  >
    ...
    <div v-else-if="!activeClasses.length" class="dialog-empty">
      ...
      <p>还没有创建班级</p>
      <p class="dialog-empty__hint">请先在「班级管理」中创建班级，再将资源发送给学生</p>
    </div>
