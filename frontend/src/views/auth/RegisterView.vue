...
async function handleSubmit() {
  ...
  try {
    await register({ email: form.email, phone: form.phone || undefined, password: form.password, grade: form.grade, subjects: form.subjects })
  ...
  <div class="auth-page">
    <div class="auth-card auth-card--wide">
      <div class="auth-card__header">
        <h2>创建账号</h2>
        <p>{{ ['填写基本信息', '选择年级', '选择学科'][step] }}</p>
      </div>

      <div class="step-bar">
        <div v-for="(_, i) in 3" :key="i" class="step-dot" :class="{ active: i <= step, done: i < step }" />
      </div>
      ...
      <div class="auth-card__actions">
        ...
        <el-button v-if="step < 2" type="primary" size="large" @click="step++">下一步</el-button>
        <el-button v-if="step === 2" type="primary" size="large" :loading="loading" @click="handleSubmit">完成注册</el-button>
      </div>
