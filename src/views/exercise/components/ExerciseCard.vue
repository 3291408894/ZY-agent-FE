...
const renderedQuestion = computed(() => renderLatex(props.exercise.question))
// 规范化选项：去除已有字母前缀，统一添加 A. B. C. D.
const normalizedOptions = computed(() => {
  const raw = props.exercise.options || []
  return raw.map((opt, i) => ({
    value: opt,
    label: `${String.fromCharCode(65 + i)}. ${opt.replace(/^[A-D][.)、]\s*/i, '')}`,
  }))
})
const renderedOptions = computed(() => normalizedOptions.value.map(o => renderLatex(o.label)))
const renderedAnswer = computed(() => renderLatex(props.exercise.answer || ''))
const renderedAnalysis = computed(() => renderLatex(props.exercise.analysis || ''))
const renderedGradeAnswer = computed(() => renderLatex(props.gradeResult?.correct_answer || ''))
const renderedUserAnswer = computed(() => renderLatex(props.userAnswer || ''))
...
// 选择题选择（el-radio-group 用 model-value + update）
const localChoiceAnswer = computed({
...
<template>
  <div class="exercise-card" :class="{ 'exercise-card--graded': isGraded && !gradeResult?.is_correct, 'exercise-card--correct': isGraded && gradeResult?.is_correct }">
    ...
    <!-- 题目内容 -->
    <div class="exercise-card__question" v-html="renderedQuestion"></div>
    ...
    <div v-if="exercise.question_type === 'choice' && exercise.options?.length" class="exercise-card__choices">
      <div
        v-for="(opt, optIdx) in normalizedOptions"
        :key="optIdx"
      ...
      >
        <template v-if="!showCorrectAnswer">
          ...
          <el-radio-group v-model="localChoiceAnswer">
            <el-radio :value="opt.value">
              <span v-html="renderedOptions[optIdx]"></span>
            </el-radio>
          </el-radio-group>
        </template>
        <template v-else>
          <!-- review / graded：只读展示 -->
          <span :class="getOptionClass(opt.value)" v-html="renderedOptions[optIdx]"></span>
          <span v-if="getOptionClass(opt.value) === 'option--correct'" class="option-badge">✓ 正确答案</span>
          <span v-if="getOptionClass(opt.value) === 'option--wrong'" class="option-badge option-badge--wrong">✗ 你的选择</span>
        </template>
      </div>
    </div>
    ...
    <div v-if="showCorrectAnswer" class="exercise-card__answer-section">
      ...
      <div v-if="isGraded" class="exercise-card__answer-compare">
        <div class="exercise-card__user-answer" v-if="userAnswer">
          ...
          <span :class="gradeResult?.is_correct ? 'text-correct' : 'text-wrong'" v-html="renderedUserAnswer"></span>
        </div>
        <div class="exercise-card__correct-answer">
          <span class="answer-label">正确答案：</span>
          <span class="text-correct" v-html="renderedGradeAnswer"></span>
        </div>
      </div>
